...
2. If you are using Google Cardboard SDK in your app, you should remove the `-ObjC` flag from the link flags in Xcode and add `-force_load $(BUILT_PRODUCTS_DIR)/libSoomlaiOSCore.a` and `-force_load $(BUILT_PRODUCTS_DIR)/libSoomlaiOSProfile.a` to `Other Linker Flags`.

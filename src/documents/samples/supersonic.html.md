...
## Use Cases
<ul>
  <li style="color:black">Reward coins on RewardedVideo/OfferWall ads completion.</li>
  <li style="color:black">Show an Interstitial ad when a level ends (Unity only).</li>
  <li style="color:black">Offer the user to watch a video or complete an offer to receive currency when he has insufficient funds to purchase an item.</li>
</ul>
<br>
...
public class MainGame : MonoBehaviour {
    ...
    void OnGUI(){
        ...
        if (!isRewardedVideoReady)
            GUI.enabled = false;
        if(GUILayout.Button("Earn X coins by watching a video!"))
    ...
    void onLevelEnded(Level level){
        // A good place to show an Interstitial ad is when the level is ending
    ...
    void onRVRewarded (int amount){
        // Reward the user with the amount of coins rewarded from the watching the video
...
public class MainGame : MonoBehaviour {
    ...
    <div role="tabpanel" class="tab-pane" id="sample-cocos2dx"><br><b>Coming soon..</b></div>
...
@implementation ViewController
    ...
    - (IBAction)buyShieldBtn:(id)sender {
        ...
        @catch (InsufficientFundsException *exception) {
            // Not enough coins to buy a shield, show some free options to the users to earn more coins
    ...
    - (BOOL)supersonicOWDidReceiveCredit:(NSDictionary *)creditInfo {
        [StoreInventory giveAmount:[[creditInfo valueForKey:@"credits"] intValue] ofItem:@"coins_currency"];
        // Return true if you successfully rewarded the user
        return true;
    }

    - (void)supersonicOWInitSuccess{ self.offerwallBtn.enabled = true; }
    - (void)supersonicOWInitFailedWithError:(NSError *)error { self.offerwallBtn.enabled = false; }
    - (void)supersonicRVAdAvailabilityChanged:(BOOL)hasAvailableAds { self.rewardedVideoBtn.enabled = hasAvailableAds; }
...
2. [Login](https://www.supersonicads.com/partners/login) to the Supersonic Dashboard and [add your application](http://developers.supersonic.com/hc/en-us/articles/200772061-Adding-Your-App).

3. Follow the integration instructions for your platform: <b>|</b>
   [Unity](http://developers.supersonic.com/hc/en-us/articles/201527091-Getting-started-with-the-Supersonic-Unity-Plugin-) <b>|</b>
   [iOS](http://developers.supersonic.com/hc/en-us/articles/201328522-Getting-Started-with-the-Supersonic-iOS-SDK) <b>|</b>
   [Android](http://developers.supersonic.com/hc/en-us/articles/201481051-Getting-Started-with-the-Supersonic-Android-SDK) <b>|</b>

4. If you didn't do so already, Integrate Soomla Store (+Soomla Levelup if you want to use the LevelUp example in Unity)

    - Unity  : [Store](http://know.soom.la/unity/store/store_gettingstarted/) <b>|</b> [LevelUp](http://know.soom.la/unity/levelup/levelup_gettingstarted/)
    - iOS    : [Store](http://know.soom.la/ios/store/store_gettingstarted/)
    - Android: [Store](http://know.soom.la/android/store/store_gettingstarted/)
